    ...
    Tensor<Ty> ttm(const Tensor<Ty> &A, const Tensor<Ty> &M, size_t n) {
        if (A.distribution()->type() ==
            ...
            for(size_t i = 0; i < kParN; i++){
                ...
                Summary::start("Operator<double>::matmulGeneral()", 2 * (long long) remain_size *
                        (long long)(row_local_begin[k + 1] - row_local_begin[k]) *(long long) col_local_size);
                ...
                Summary::end("Operator<double>::matmulGeneral()");
                if (i != 0) {
                    A.comm()->wait(request_send);
                    A.comm()->wait(request_recv);
